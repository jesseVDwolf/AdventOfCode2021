...
class TwentyOneDayFive(cookieHeader: Map[String, String]) extends Solution {
  ...
  def solve: Unit = {
    ...
    inputRanges.foreach{ (range: Array[Array[Int]]) => {
      ...
      val pointsInRange : Array[Array[Int]] = {
        if (range(0)(1) == range(1)(1)) {
          getPointRange(range(0)(0), range(1)(0))
            .toArray
            .map{ (x: Int) => Array(x, range(0)(1)) }
        }
        else {
          getPointRange(range(0)(1), range(1)(1))
            .toArray
            .map{ (y: Int) => Array(range(0)(0), y) }
        }
      }
    ...
    val numberPointsLinesOverlap = gridLines2DMatrix.flatten.count(_ > 1)
      ...
      val pointsInRange : Array[Array[Int]] = {
        if (range(0)(1) == range(1)(1)) {
          ...
          if ((range(0)(1) - range(1)(1)) == (range(0)(0) - range(1)(0))) {
            (xPointRange zip yPointRange)
              .toArray
              .map{ (xy: (Int, Int)) => Array(xy._1, xy._2) }
          }
          else {
            (xPointRange zip yPointRange.reverse)
              .toArray
              .map{ (xy: (Int, Int)) => Array(xy._1, xy._2) }
          }
        }
